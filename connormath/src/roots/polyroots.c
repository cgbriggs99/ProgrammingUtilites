...
int poly_roots(double *coefs, int len, double *out, double conv) {
  ...
  if(len == 2) { // Linear
    ...
    if(del0 == 0 && del1 == 0) {
      ...
      C = cbrt((del1 + sqrt(del1 * del1 - 4 * del0 * del0 * del0)) / 2);
    }
    _Complex double r0, r1, r2;
    _Complex double xi = CMPLX(-0.5, sqrt(3) / 2);
    r0 = -(b + C + del0 / C) / (3 * a);
    r1 = -(b + C * xi + del0 / (C * xi)) / (3 * a);
    r2 = -(b + C / xi + del0 * xi / C) / (3 * a);
    int pos = 0;
    if(cimag(r0) == 0) {
      out[pos] = creal(r0);
      pos++;
